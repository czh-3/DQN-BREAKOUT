...
class Agent(object):
    ...
    def GetPriority(self, nowstate, action, reward, done, nextstate):
        with torch.no_grad():
            v1 = self.__policy(nowstate.float()).gather(1, action)
            v2 = self.__target(nextstate.float()).max(1).values.detach()
            e = reward + self.__gamma * v2.unsqueeze(1) * (1. - done)
            return e - v1
